...
export const submitExam: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        const resp: ReturnResponse = {
            ...
            data: { score, total, reportId: data._id }
        }
...
export const doesQuizExist = async (quizId: Mongoose["Types"]["ObjectId"]) => {
    const quiz = await Quiz.findById(quizId);
    if (!quiz)
        return false;
    return true;
...
export const isValidAttempt = async (attemptedQuestion: {}, quizId: Mongoose["Types"]["ObjectId"]) => {
    const quiz = await Quiz.findById(quizId);
    const questions = Object.keys(attemptedQuestion);
    const attemptQue = Object.keys(attemptedQuestion);
    if (attemptQue.length != questions.length) {
        return false
    }
    let flag = 0;
    attemptQue.forEach((e) => {
